...
public class FortifyCollectorTask extends CollectorTask<FortifyCollector> {
	...
	public FortifyCollector getCollector() {
		...
		Configuration config = configurationRepository.findByCollectorName(FortifyCollector.NICE_NAME);
		
		if(null != config){
			for (Map<String, String> fortifyServer : config.getInfo()) {
                serverUrls.add(fortifyServer.get("url"));
            }
		}
	...
	public void collect(FortifyCollector collector) {
		...
		if (config == null) {
        	LOG.info("No Fortify Servers Configured");
            return;
        }

		try {
			config.decryptOrEncrptInfo();
			fortifyClient.setServerDetails(config.getInfo());
			...
			for (String instanceUrl : collector.getFortifyServers()) {
				...
				try {
					...
					// Since this is rest exception in getting the basic
					// projects, we set error
					CollectionError error = new CollectionError("400", "Fortify Failed to Respond");
					for (FortifyProject project : filteredExistingProjects) {
						project.getErrors().add(error);
					}
					fortifyProjectRepository.save(filteredExistingProjects);
			...
			LOG.error("Decryption error. Wrong Decrpt Key!");
		}
		...
		for (FortifyProject application : enabledProjects) {
			...
			try {
				...
				if (versionJson != null) {
					...
					error = new CollectionError("Fortify project not found", application.getProjectName());
				}
	...
	private boolean isNewCodeQualityData(FortifyProject application, long timeStamp) {
		CodeQuality existingReport = codeQualityRepository.findByCollectorItemIdAndTimestamp(application.getId(), timeStamp);
